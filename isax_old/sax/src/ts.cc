...
float ts_euclidean_distance_avx512(ts_type * t, ts_type * s) {


    float dis = 0;

    int len = TS_LENGTH / 8 * 8;
...
//    std::cout<<2<<std::endl;
//
//    for (int i=0;i<TS_LENGTH;i+=16) {
//        std::cout<<3<<std::endl;
//        a = _mm512_loadu_ps(t + i);
//        b = _mm512_loadu_ps(s + i);
//        std::cout<<3<<std::endl;
//        __m512 c = _mm512_sub_ps(a, b);
//        __m512 d = _mm512_mul_ps(c, c);
//        std::cout<<3<<std::endl;
//        xfsSum = _mm512_add_ps(xfsSum, d);
//    }
//    std::cout<<4<<std::endl;
//    return _mm512_reduce_add_ps(xfsSum);
}

float ts_euclidean_distance(ts_type* t, ts_type* s) {
    float dis = 0;
...
float my_ts_euclidean_distance(float *t, float *s, int size) {
    float distance = 0;
