...
void paa_from_ts(const ts_type *ts_in, ts_type *paa)
{
    ...
    int i,s;
    for (i = 0, s = 0; s < SEGMENTS - 1; i += 1, s ++) {
        paa[s] = 0;
        for (int j = i; j < i + 1; j ++ ) {
            paa[s] += ts_in[j];
        }
...
void saxt_from_ts(const ts_type *ts_in, saxt_type *saxt_out)
{
    ...
    float paa[SEGMENTS];
    int i,s;
    for (i = 0, s = 0; s < SEGMENTS - 1; i += NUM_PER_SEGMENT, s ++) {
        paa[s] = 0;
    ...
    for (int ii = 0; ii < BIT_CARDINALITY; ii ++ ) {
        saxt_type a_word_saxt = 0;
        sax_type bit = 1 << (BIT_CARDINALITY - ii - 1);
        for (int j = 0; j < SEGMENTS; j ++ ) {
            a_word_saxt |= ( ((uint64_t)((sax.sax[j] & bit) >> (BIT_CARDINALITY - ii - 1))) << j);
        }
        saxt_out[BIT_CARDINALITY - ii - 1] = a_word_saxt;
    }
}
...
float min_dist_paa_to_sax(const ts_type *paa, SAX sax_) {
    ...
    return dis*NUM_PER_SEGMENT*MULTIPLY_RATIO;
}
...
/**
 * 根据saxt和card，计算相同前缀的下界距离，前缀查表的上下界，和将前缀补齐0的下界和补齐1的上界一样
 * 查的表sax_a需要固定bit_cardinality = 8
 */
...
/**
 * 利用saxt左右边界，计算相同前缀的下界距离，前缀查表的上下界，和将前缀补齐0的下界和补齐1的上界一样
 * 查的表sax_a需要固定bit_cardinality = 8
 * 相同前缀是按位考虑的，而不是按word考虑
 */
float min_dist_paa_to_saxt(const ts_type *paa, SAXT saxt_lb, SAXT saxt_ub) {
    exit(123);
//    assert(BIT_CARDINALITY == 8);
//    int cod_word = 0;   // 哪几个word相等
//    int cod_bit = 0;  // 不相等的word里哪几位相等
