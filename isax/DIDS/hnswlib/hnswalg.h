...
//                    if (candidate_id == 0) continue;
    ...
    */
    ...
    searchKnn(const void *query_data, size_t k, BaseFilterFunctor* isIdAllowed = nullptr) const {
        std::vector<std::pair<dist_t, uint32_t >> real_result(k);
...
//        if (cur_element_count == 0) return result;
        ...
        for(int i=0;i<k;i++) {
            real_result[i] = {result.top().first, result.top().second};
            result.pop();
        }
    ...
    std::vector<std::pair<dist_t, uint32_t >>
    searchKnn(const void *query_data, size_t k, uint32_t enter_id, BaseFilterFunctor* isIdAllowed = nullptr) const {
        std::vector<std::pair<dist_t, uint32_t >> real_result(k);
        std::priority_queue<std::pair<dist_t, labeltype >> result;
//        if (cur_element_count == 0) return result;
